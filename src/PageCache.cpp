...
Span* PageCache::NewSpan(size_t k)
{
    ...
    if (k >= PAGE_NUM)
    {
        ...
        if (!_id2SpanMap.Ensure(span->_pageID, span->_n))
        {
            // 处理失败（理论上不会，除非地址越界）
            abort();
        }
        _id2SpanMap.set(span->_pageID, span);
    ...
    if (!_spanLists[k].Empty())
    {
        ...
        for (PageID i = 0; i < span->_n; ++i)
        {
            ...
            _id2SpanMap.set(span->_pageID + i, span);
        }
    ...
    for (int i = k + 1; i < PAGE_NUM; ++i)
    {
        if (!_spanLists[i].Empty())
        {
            ...
            if (!_id2SpanMap.Ensure(nSpan->_pageID, nSpan->_n) ||
                !_id2SpanMap.Ensure(kSpan->_pageID, kSpan->_n))
            {
                // 处理失败（理论上不会，除非地址越界）
                abort();
            }
            // _id2SpanMap[nSpan->_pageID] = nSpan;
            _id2SpanMap.set(nSpan->_pageID, nSpan);
            // _id2SpanMap[nSpan->_pageID + nSpan->_n - 1] = nSpan;
            _id2SpanMap.set(nSpan->_pageID + nSpan->_n - 1, nSpan);

            for (PageID i = 0; i < kSpan->_n; ++i)
            {
                // _id2SpanMap[kSpan->_pageID + i] = kSpan;
                _id2SpanMap.set(kSpan->_pageID + i, kSpan);
            }
...
Span* PageCache::MapObj2Span(void* obj)
{
    ...
    // std::unique_lock<std::mutex> lock(_pageMtx); // 使用基数树后不再需要锁
    // 通过map找到该页号对应的span，应该是一定能找到的，否则出错
    // if (!_id2SpanMap.count(id))
    auto* span = _id2SpanMap.get(id);
    if (span == nullptr)
    {
        assert(false);
        return nullptr;
...
void PageCache::ReleaseSpanToPageCache(Span* span)
{
    ...
    _id2SpanMap.set(span->_pageID + span->_n - 1, span);
}